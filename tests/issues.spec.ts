...
import { TASK_DESCRIPTION, NOT_FOUND_QUERY, generateTaskTitle } from '../utils/dataTest';
...
test.describe('Страница задач', () => {
  ...
  test.describe('Создание задачи', () => {
    test('Создание новой задачи', async ({ issuesPage, taskModal }) => {
  ...
  test.describe('Открытие карточки задачи', () => {
    test('Открытие карточки созданной задачи', async ({ issuesPage, taskModal }) => {
      const taskTitle = generateTaskTitle();
  ...
  test.describe('Поиск задачи', () => {
    test('Поиск существующей задачи', async ({ issuesPage, taskModal }) => {
      const taskTitle = generateTaskTitle();
      ...
      await test.step('Убедиться, что задача создана', async () => {
        await issuesPage.expectTaskVisible(taskTitle);
      });
      ...
      await test.step('Ввести запрос в поиск', async () => {
        await expect(issuesPage.searchInput).toBeVisible();
        await issuesPage.searchInput.fill(taskTitle);
      });
